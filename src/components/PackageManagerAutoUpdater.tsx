...
export function PackageManagerAutoUpdater(t0) {
    ...
    t1 = async () => {
      ...
      if (maxVersion && latest && gt(latest, maxVersion)) {
        ...
        if (gte("1.0.0-snapshot", maxVersion)) {
          logForDebugging(`PackageManagerAutoUpdater: current version ${"1.0.0-snapshot"} is already at or above maxVersion ${maxVersion}, skipping update`);
          setUpdateAvailable(false);
          return;
        }
        latest = maxVersion;
      }
      const hasUpdate = latest && !gte("1.0.0-snapshot", latest) && !shouldSkipVersion(latest);
      setUpdateAvailable(!!hasUpdate);
      if (hasUpdate) {
        logForDebugging(`PackageManagerAutoUpdater: Update available ${"1.0.0-snapshot"} -> ${latest}`);
      }
    };
    $[0] = t1;
  } else {
  ...
  let t4;
  if ($[3] !== verbose) {
    t4 = verbose && <Text dimColor={true} wrap="truncate">currentVersion: {"1.0.0-snapshot"}</Text>;
    $[3] = verbose;
    $[4] = t4;
  } else {
    t4 = $[4];
