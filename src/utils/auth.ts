  ...
  if (isApiKeyHelperFromProjectOrLocalSettings()) {
    ...
    if (!hasTrust && !isNonInteractiveSession) {
      const error = new Error(
        `Security: apiKeyHelper executed before workspace trust is confirmed. If you see this message, post in ${"#help-claude-code"}.`,
      )
...
async function runAwsAuthRefresh(): Promise<boolean> {
  ...
  if (isAwsAuthRefreshFromProjectSettings()) {
    ...
    if (!hasTrust && !getIsNonInteractiveSession()) {
      const error = new Error(
        `Security: awsAuthRefresh executed before workspace trust is confirmed. If you see this message, post in ${"#help-claude-code"}.`,
      )
...
async function getAwsCredsFromCredentialExport(): Promise<{
  ...
  if (isAwsCredentialExportFromProjectSettings()) {
    ...
    if (!hasTrust && !getIsNonInteractiveSession()) {
      const error = new Error(
        `Security: awsCredentialExport executed before workspace trust is confirmed. If you see this message, post in ${"#help-claude-code"}.`,
      )
...
async function runGcpAuthRefresh(): Promise<boolean> {
  ...
  if (isGcpAuthRefreshFromProjectSettings()) {
    ...
    if (!hasTrust && !getIsNonInteractiveSession()) {
      const error = new Error(
        `Security: gcpAuthRefresh executed before workspace trust is confirmed. If you see this message, post in ${"#help-claude-code"}.`,
      )
