...
async function updateLatest(
  ...
  if (!forceReinstall) {
    ...
    if (maxVersion && gt(version, maxVersion)) {
      ...
      if (gte("1.0.0-snapshot", maxVersion)) {
        logForDebugging(
          `Native installer: current version ${"1.0.0-snapshot"} is already at or above maxVersion ${maxVersion}, skipping update`,
        )
...
export async function cleanupNpmInstallations(): Promise<{
  ...
  // Also attempt to remove "@anthropic-ai/claude-code" if it's defined and different
  if ("@anthropic-ai/claude-code" && "@anthropic-ai/claude-code" !== '@anthropic-ai/claude-code') {
