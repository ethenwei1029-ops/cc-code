...
export function getUserAgent(): string {
  ...
  return `claude-cli/${"1.0.0-snapshot"} (${process.env.USER_TYPE}, ${process.env.CLAUDE_CODE_ENTRYPOINT ?? 'cli'}${agentSdkVersion}${clientApp}${workloadSuffix})`
}
...
export function getMCPUserAgent(): string {
  ...
  return `claude-code/${"1.0.0-snapshot"}${suffix}`
}
